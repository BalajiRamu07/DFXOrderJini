...
angular.module('ebs.controller')
    ...
    .controller("CustomerDetailCtrl", function ($scope, $filter, $http, $modal, $window, toastr, Settings, $interval, $sce, $mdDialog, $location, $routeParams) {
        console.log("Hello From Customer Detail Controller .... !!!!");
        ...
        $scope.showDoorno = true;
        $scope.showLandmark = true;
        $scope.showGcode = true;
        ...
        $http.get("/dash/sellers").success(function(result){
            if(result && result.seller.length)
                $scope.userNames = [];
            for(var i = 0; i < result.seller.length; i++){
                if(result.seller[i])
                    $scope.userNames.push({sellername : result.seller[i].sellername, sellerphone : result.seller[i].sellerphone});
            }
        })
        ...
        $scope.showListDealerDetails = function (dealer, type) {
            if (dealer) {
                ...
                $scope.editedDealer.Plant = dealer.Plant;
                ...
                $scope.editedDealer.customerCategory = dealer.customerCategory || '';
              ...
              //  $scope.editedDealer.assignedStockist = stockist;
                ...
                $scope.editedDealer.account_id=dealer.account_id;
                $scope.editedDealer.account_name=dealer.account_name;
                $scope.editedDealer.customer_group=dealer.customer_group;
                $scope.editedDealer.Doorno=dealer.Doorno;
                $scope.editedDealer.Landmark=dealer.Landmark;
                $scope.editedDealer.GoogleDigitalCode=dealer.GoogleDigitalCode;
                ...
                console.log($scope.storesDisp.account_id);
                $http.get("/dash/customerStores/"+ $scope.storesDisp.account_id).success(function (res) {
        ...
        $scope.renderStoreEntryMap = function (order, view) {
            ...
            }else  console.log("Google Maps Not available ---> ", google);
        }

        $scope.editDealerDetails = function (dealer, type, newDealer) {
            console.log("entered edit dealer")
                            ...
                            if ($scope.pinCodeMadatory) {
                                if (newDealer.Pincode && newDealer.Address && newDealer.City) {
                                    ...
                                    if (newDealer.assignedSellers.length) {
                                        // console.log("qwerty",$scope.getSellerName(newDealer.assignedSellers[0]))
                                        newDealer.Seller2 = '';
                                        newDealer.SellerName2 = '';
                                        newDealer.Seller3 = '';
                                ...
                                if (newDealer.assignedSellers.length) {
                                    // console.log("qwerty1234",getSellerName(newDealer.assignedSellers[0]))
                                    newDealer.Seller2 = '';
                                    newDealer.SellerName2 = '';
                                    newDealer.Seller3 = '';
                                    newDealer.SellerName3 = '';
                                    if (newDealer.assignedSellers.length == 1) {
                                        ...
                                        for (var i = 2; i < newDealer.assignedSellers.length; i++) {
                                            newDealer.Seller3.push(newDealer.assignedSellers[i]);
                                            newDealer.SellerName3.push($scope.getSellerName(newDealer.assignedSellers[i]));
                                        }
                                    }
                                }else{
                                    newDealer.Seller = '';
        ...
        $http.get("/dash/stores/" + $routeParams.id)
            ...
            $scope.showHideAddress = function (response) {
                ...
                if (response.Doorno === "" || response.Doorno === undefined || response.Doorno === null) {
                    $scope.showDoorno = false;
                }
                if (response.Landmark === "" || response.Landmark === undefined || response.Landmark === null) {
                    $scope.showLandmark = false;
                }
                if (response.GoogleDigitalCode === "" || response.GoogleDigitalCode === undefined || response.GoogleDigitalCode === null) {
            ...
            editDealerAddress_autocomplete.addListener('place_changed', function () {
                ...
                for (var i = 0; i < newplace.address_components.length; i++) {
                    ...
                    if(newplace.address_components[i].types[0] == 'plus_code'){
                        jaddress='';
                        scope.editedDealer.GoogleDigitalCode = newplace.address_components[i].long_name.toString();
                    }
                    else{
                ...
                $('#editDealerGoogleDigitalCode').val(scope.editedDealer.GoogleDigitalCode);
        ...
        $scope.empty = function (type) {
            ...
            if (type == 'new') {
                if ($scope.dealer.Address == "" || $scope.dealer.Address == undefined) {
                    ...
                    $('#editDealerGoogleDigitalCode').val('');
                    ...
                    $scope.dealer.GoogleDigitalCode = "";
                    $scope.dealer.Doorno = "";
                    $scope.dealer.Landmark = "";
            ...
            if (type == 'edit') {
                if ($scope.editedDealer.Address == "" || $scope.editedDealer.Address == undefined) {
                    ...
                    $scope.editedDealer.GoogleDigitalCode = "";
                    $scope.editedDealer.Doorno = "";
                    $scope.editedDealer.Landmark = "";
                }
            }
