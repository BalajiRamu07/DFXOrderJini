    ...
    .controller("CustomersCtrl",function ($scope, $filter, $http, $routeParams, $window, toastr, Settings, $location) {
        ...
        $scope.redirectTo = code => {
            switch($scope.redirect){
                case "enquiry" : {
                    $location.search({});
                    $location.path("/enquiry/create/" + code);
                    break;
                }
                case "service" : {
                    $location.search({});
                    $location.path("/service/new/ticket/" + code);
                    break;
                }
                case "order" : {
                    $location.search({});
                    $location.path("/order/create/" + code);
                    break;
                }
                default : {
                    $location.path("/customer-detail/" + code);
                }
            }
        }
            ...
            for (var i = 0; i < result.length; i++) {
                for (var j = 0; j < header.length; j++) {
                    ...
                    try {
                        ...
                        if (temp == 'Doorno') {
                            console.log('Result temp', result[i][temp]);
                            if (result[i][temp] && result[i][temp].length) {
                                if ((result[i][temp]).toString().indexOf('/') != -1) {
                                    if((result[i][temp]).toString().indexOf('#') == -1) { // if there is no #
                                        var quotesWrapped = "# " + result[i][temp];
                                        console.log('inside if # is not present', result[i][temp]);
                                    }
                                    console.log('quotes wrapped data inside if / ', quotesWrapped);
                                    result[i][temp] = quotesWrapped
                                }
                                if((result[i][temp]).toString().indexOf('-') != -1 && ((result[i][temp]).toString().indexOf('#') == -1)) {
                                    var quotesWrapped = "# " + result[i][temp];
                                    result[i][temp] = quotesWrapped
                                    console.log('quotes wrapped data inside if - ', quotesWrapped);
                                }
