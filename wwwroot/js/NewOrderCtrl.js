...
angular.module('ebs.controller')
    ...
    .controller("NewOrdersCtrl",function ($scope, $filter, $http, $modal,$routeParams, $window,Settings, toastr, $interval,$sce,$mdDialog,$location) {
        ...
        $scope.DealerSearchFilter = function(data) {
            if($scope.newOrderItemList.length){
                Settings.confirmPopup('','Customer change will remove all selected items from cart', (res) => {
                    if (res) {
                        $scope.handleCancelNewOrder();
                        $scope.$apply();
                        searchDealer(data);
                ...
                if($scope.data.newOrderStore.class && $scope.dealerClasses){
                    for(var i=0;i<$scope.dealerClasses.length;i++){
                        if($scope.dealerClasses[i].name == $scope.data.newOrderStore.class){
                            $scope.orderPriceList = $scope.dealerClasses[i].priceList;
                            console.log('This is orderPList based on dealer class', $scope.orderPriceList);
                        }
                    }
                }
        ...
        $scope.submitOrder = function(flag){
            ...
            for(var i=0; i < $scope.newOrderItemList.length; i++) {
                ...
                itemAsStoredInMongo = {
                    ...
                    "customerCategory": $scope.data.newOrderStore.customerCategory ? $scope.data.newOrderStore.customerCategory : '',
                    "itemcode": $scope.newOrderItemList[i].itemDetails.itemCode,
                    "hsn_code": $scope.newOrderItemList[i].itemDetails.hsn_code,
